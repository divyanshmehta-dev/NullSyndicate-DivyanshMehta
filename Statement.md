# Problem Statement:-

## Title:-
**Smart Campus Navigation System for VIT Bhopal**

## Problem:-
Large university campuses like VIT Bhopal have widely spread academic blocks, hostels, sports facilities, food courts, and amenities and this is the main reason why new students, freshers, visitors and even guests often struggle to:

- Locate specific buildings, departments or facilities quickly.
- Estimate walking time between their current location and their destination.
- Get quick answers to simple campus-related queries without searching through multiple sources or asking around.

Existing solutions like Google Maps are not made for the campus environment — they don't show internal landmarks, walking times between specific campus points or categorized facilities relevant to student life.

## Proposed Solution:-
We built a **Smart Campus Navigator** — a lightweight, browser-based web application that provides:

1. An **interactive campus map** with all major locations pinned and color-coded by category.
2. A **search and filter system** to instantly locate places by name, category, or landmark.
3. **Walking-time estimates** from key reference points (Hostels, AB-1) to any location.
4. A **conversational chatbot assistant** that answers natural-language questions about campus locations using keyword matching.
5. A **gallery/slideshow** to help newcomers visually familiarize themselves with the campus.

## Target Users:-
- Freshers and new students during orientation.
- Campus visitors and guest faculty.
- Existing students looking for quick facility information.

## Impact:-
- Reduces the time newcomers spend locating places on campus.
- Improves the onboarding experience during orientation week.
- Provides a single, easy-to-use reference tool without needing app installation or internet-based mapping services.

## Future Scope:-
- Real-time crowd/occupancy data for canteens and common areas.
- Integration with the official VIT Bhopal timetable/ERP system for class-specific navigation.
- Mobile app version with offline map support.
